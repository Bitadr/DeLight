...
def test_mlp(
    ...
    while epoch_counter < n_epochs and (not done_looping):
        ...
        for minibatch_index in xrange(n_train_batches):
            ...
            if (iter + 1) % validation_frequency == 0:
        		...
        		print('epoch %i, validation error %f %%' % (epoch_counter, this_validation_errors * 100.))
        		if this_validation_errors < best_validation_errors:
					...
					best_iter = iter
					best_params = param
					test_losses2 = [test_model(i) for i in xrange(n_test_batches)]
					test_score2 = float(np.sum(test_losses2)) / test_set_x.get_value(borrow=True).shape[0]
    ...
    print(('Optimization complete. Best validation score of %f %% '
          ...
          (best_validation_errors * 100., epoch_counter, test_score2 * 100.))
    ...
    test_model_final3 = theano.function(inputs=[index],
            outputs=classifier.layers[-1].y_pred,
            givens={
                x: test_set_x,
                y: test_set_y},
    ...
    for i in xrange(test_model_final(1).shape[0]):
    	if (test_model_final3(1)[i] == test_true_label[i]):
    		counter += 1.
    print counter
    print ((test_model_final3(1).shape[0] - counter)/test_model_final3(1).shape[0])*100.
